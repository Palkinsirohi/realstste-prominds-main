...
const Navbar = () => {
  ...
  const getLogoSize = () => {
    ...
    if (screenSize.ismediumDesktop) return 'h-12 w-[170px]';
  ...
  const getNavGap = () => {
    ...
    if (screenSize.isLargerDesktop) return 'gap-8';
    if (screenSize.isDesktop) return 'gap-2';
    if (screenSize.ismediumDesktop) return 'gap-1';
    ...
    <>
      <div className="flex w-full items-start">
        ...
        <div className={`pt-14 md:pt-16 lg:pt-20 w-full`}>
          {/* Sidebar - now fixed */}
          {location.pathname !== '/privacy-policy' && <Sidebar />}
